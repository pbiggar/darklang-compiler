---
format: 1
event-id: 01a03d9a74dc7e3eb8f3850e071d6108
entity-id: 01a03c31c1b47a349667cc719ddddbaa
entity-kind: issue
event-type: trial-result
occurred-at: 2026-08-26T10:25:27.7725633+00:00
author: worker:65eb66d2e522:1247164:01a03ba6317d7aa6b0ca4b7870dfe6eb
previous: 01a03c31c1b874ff8d93a4cc6094b5c0
attempt: 01a03d5bc0417e14b5901427a3835685
candidate: f065e2d0c90a3c32766f96b685472ad4a5ff65f2
...
- **No candidate code or checkpoint was changed** — **Reason:** This was a measurement-only trial and HEAD remains at `f065e2d0c90a3c32766f96b685472ad4a5ff65f2`.
- **The pinned revisions were measured with the registered Pass Optimization verifier** — **Reason:** The bounded contract required one warmup and three comparable ANF→MIR samples on matmul.
- **The candidate was classified as no-improvement and left available** — **Reason:** Both medians were 0.8 ms, below the required 1.0 ms clear-win threshold, while MIR and executable equality also failed.

## How it works

The candidate replaces eligible private dense `Dict<Int64,Int64>` matrices with contiguous raw storage, but the required focused pass measurement found no ANF→MIR compile-time improvement: baseline and candidate medians were both 0.8 ms. Runtime output was byte-identical, but normalized MIR and executable bytes differed, causing the registered verifier to reject with `correctness-mismatch`. No five-pair confirmation was warranted because the result was neutral rather than ambiguously above threshold. The transformation reduces eligible matrix access from HAMT traversal/persistent updates to constant-time indexed loads/stores, but adds a narrow whole-program recognizer, bounds paths, raw allocation lifetime handling, and code-size risk. Because the focused acceptance contract was not met, no retained solution was chosen; the candidate checkpoint remains intact for DCB's later repository gates.

## Algorithm

1. Identify the canonical closed matmul call graph and exact monomorphic `Dict<Int64,Int64>` signatures.
2. Prove literal dimensions, dense zero-based construction, bounded indexing, no aliases, no escapes, and no unknown callers.
3. Reject lowering whenever any proof is absent, preserving ordinary Dict semantics.
4. For eligible matrices, allocate length-prefixed contiguous Int64 buffers.
5. Replace eligible HAMT reads and persistent writes with bounds-checked eight-byte raw accesses.
6. Preserve the original generation, dot-product, output traversal, and checksum recurrences.
7. Release each large raw buffer explicitly at the end of its ownership region.
...
| --- | --- | --- | ---: | ---: | --- |
| focused optimization case | GIT_CONFIG_COUNT=1 GIT_CONFIG_KEY_0=safe.directory GIT_CONFIG_VALUE_0=/workspace/c4d-for-dcb/.git/dcb2/worktrees/01a03d5bc0417e14b5901427a3835685 python3 agents/001-pass-optimization/tools/pass_optimization.py verify --pass "ANF -> MIR" --benchmark benchmarks/problems/matmul/dark/main.dark --baseline 39d6c0d0cfd2df07b2f5ed80f4d9315e6da88e50 --candidate f065e2d0c90a3c32766f96b685472ad4a5ff65f2 --ir mir --focused-check executable | Checks selected-pass timing, deterministic normalized MIR, executable bytes, and runtime-output equality for pinned revisions. | ANF→MIR samples [0.8, 1.0, 0.8] ms; median 0.8 ms; normalized MIR SHA-256 `a02853bc60fe93b8abdd45b36bf6dc0b746a8f964f308a3fb572e07227f1a195`; 10-byte output SHA-256 `9d4e2dc64c76a592ce3c6e141b7be6a1bcf6a3a08024641f9770be0670f02b3c`. | ANF→MIR samples [0.8, 0.7, 0.8] ms; median 0.8 ms; normalized MIR SHA-256 `27976821730803344430600a150ba32cfc2292cda78d70aeac0396822decd41b`; identical 10-byte output hash; executable bytes differed. | inconclusive |

### Benchmarks

| Benchmark | Metric / command | Before | After | Change | Result |
| --- | --- | ---: | ---: | ---: | --- |
| matmul ANF→MIR focused verification | ANF→MIR milliseconds from `GIT_CONFIG_COUNT=1 GIT_CONFIG_KEY_0=safe.directory GIT_CONFIG_VALUE_0=/workspace/c4d-for-dcb/.git/dcb2/worktrees/01a03d5bc0417e14b5901427a3835685 python3 agents/001-pass-optimization/tools/pass_optimization.py verify --pass "ANF -> MIR" --benchmark benchmarks/problems/matmul/dark/main.dark --baseline 39d6c0d0cfd2df07b2f5ed80f4d9315e6da88e50 --candidate f065e2d0c90a3c32766f96b685472ad4a5ff65f2 --ir mir --focused-check executable`; threshold 1.0 ms | Samples [0.8, 1.0, 0.8] ms; median 0.8 ms; MAD 0.0 ms. | Samples [0.8, 0.7, 0.8] ms; median 0.8 ms; MAD 0.0 ms. | 0.0 ms (0.0%); required clear-win threshold was 1.0 ms. | neutral |
...
| --- | --- | --- | --- |
| Normalized MIR equality | Registered Pass Optimization verify command above with `--ir mir` | failed | Baseline and candidate normalized MIR hashes differed. |
| Executable equality | Registered Pass Optimization verify command above with `--focused-check executable` | failed | Generated executable bytes differed between pinned revisions. |
| Runtime output equality | Execution checks embedded in the registered Pass Optimization verify command | passed | Both executables produced identical 10-byte output with SHA-256 `9d4e2dc64c76a592ce3c6e141b7be6a1bcf6a3a08024641f9770be0670f02b3c`. |
| Candidate preservation | git status --short --branch && git rev-parse HEAD | passed | Worktree remained clean and detached at `f065e2d0c90a3c32766f96b685472ad4a5ff65f2`. |

### All tests

| Command | Result | Details |
| --- | --- | --- |
| ./run-tests --ai | Passed | 8494/8494 passed in 2230.4s |

### Trial work items

| Work item | Result | Summary | Evidence |
| --- | --- | --- | --- |
| Prove matmul dense-storage eligibility | neutral | No clear focused ANF→MIR win; output matched, but MIR and executable equality failed. | Baseline [0.8,1.0,0.8] ms versus candidate [0.8,0.7,0.8] ms; medians 0.8 ms, delta 0.0 ms, threshold 1.0 ms; verifier decision `reject`. |
| Trial private dense matrix lowering | neutral | No clear focused ANF→MIR win; output matched, but MIR and executable equality failed. | Baseline [0.8,1.0,0.8] ms versus candidate [0.8,0.7,0.8] ms; medians 0.8 ms, delta 0.0 ms, threshold 1.0 ms; verifier decision `reject`. |

## Notes

Complete raw artifact: `.dcb/tool-artifacts/pass-optimization-verify-f13afa75eece`. The verifier required a command-scoped Git safe-directory override after an orchestrated-worktree ownership rejection; one stale verifier-created missing-worktree registration was pruned before the successful run. No full test suite or canonical routine benchmark was run. The successful verifier recorded current MIR evidence; it did not emit separate ANF, LIR, or ARM64 assembly comparisons, so those cannot support an improved classification.
