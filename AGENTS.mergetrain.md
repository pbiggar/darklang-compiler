...
- The train's `tests` gate records an uncontended full-suite timing while
  confined to a pair of CPUs, preserving the host CPU-count E2E behavior.
  The subsequent `test-runtime` gate compares it with the integration parent.
  Both a 10% relative increase and a 60-second absolute increase are limits;
  exceeding either fails. Contended samples never count as passing evidence.
