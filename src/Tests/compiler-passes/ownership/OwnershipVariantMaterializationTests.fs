...
let private parameter value : HIR.Parameter = { Name = "input"; Value = value }
let private definition name ownership body : Function<Leaf, HIR.ValueId> = {
    Definition = { Name = name; Body = body }
    Ownership = ownership
}
let private call target argument result : HIR.FunctionCall = { Target = target; Arguments = [argument]; Result = result }
...
let private contracts definitions = definitions |> List.map (fun definition -> definition.Definition.Name) |> Set.ofList |> hir
...
let private request caller call selection : Request<HIR.ValueId> = { Caller = caller; Call = call; Selection = selection }
...
let private testRecursiveAtomicity self () =
    ...
    |> Result.bind (fun plan ->
        ...
        | [group] ->
            ...
            let symbols = members |> List.map (fun memberDefinition -> memberDefinition.Original, memberDefinition.Function.Definition.Name) |> Map.ofList
            let contract = ownershipSemantics plan semantics
            let check memberDefinition =
                let definition = memberDefinition.Function
                let expectedTarget = if self || memberDefinition.Original = "second" then "first" else "second"
...
let private testRejectsPartialRecursiveSelection () =
    ...
    |> Result.bind (fun chosen ->
        ...
        | Error (MixedRecursiveCandidate site) when site.Caller = "first" -> Ok ()
...
let private testRejectsCallSiteErrors () =
    ...
    |> Result.bind (fun chosen ->
        ...
        let missing = { valid with Caller = "missing" }
    ...
    |> Result.bind (fun chosen ->
        ...
        |> Result.bind (fun plan ->
            ...
            | [rewrite] ->
                ...
                let shadow = { callee with Definition = { callee.Definition with Name = symbol } }
                let reserved = materialize (contracts definitions) semantics (Set.singleton symbol) definitions requests
                let declared = run (definitions @ [shadow]) requests
                match reserved, declared with
                | Error (SymbolCollision first), Error (SymbolCollision second) when first = symbol && second = symbol -> Ok ()
...
let private testRejectsMismatchedRequests () =
    let callee, definitions, firstCall, _ = singleFixture ()
    let other = { callee with Definition = { callee.Definition with Name = "other" } }
    ...
    |> Result.bind (fun chosen ->
        ...
        | Error (BoundaryMismatch "identity"), Error (BoundaryMismatch "identity"),
          Error (GroupingFailed (OwnedFunctionGroups.DuplicateFunctionName "identity")) -> Ok ()
