...
let internal testJoinCleanupPaths () =
    ...
    let func : Function = {
        Id = TestIds.functionIdForName "joinCleanup"; Name = "joinCleanup"; ReturnType = childType; ReturnOwnership = OwnedReturn
        TypedParams = [{ Id = borrowed; Type = childType }; { Id = flag; Type = AST.TBool }]
        Body = Let (owner, TupleAlloc [joinValue],
            Join (target,
                Let (TempId 15, Call (TestIds.functionIdForName "observe", [Var target.Id]),
    ...
    let rec paths joins events expr =
        ...
        | Let (_, operation, body) ->
            let event =
                ...
                | Call (name, _) when name = TestIds.functionIdForName "observe" -> Some "observe"
                | _ -> None
