        ...
        let lines =
            ...
            |> List.map (fun (i, instr, expected, actual) ->
                let instrStr =
                    ...
                    | ADD_shifted (dest, src1, src2, shift) -> $"ADD_shifted({dest}, {src1}, {src2}, LSL #{shift})"
                    ...
                    | SUB_shifted (dest, src1, src2, shift) -> $"SUB_shifted({dest}, {src1}, {src2}, LSL #{shift})"
                    | SUBS_imm (dest, src, imm) -> $"SUBS_imm({dest}, {src}, {imm})"
                    ...
                    | SXTW (dest, src) -> $"SXTW({dest}, {src})"
                    | UXTB (dest, src) -> $"UXTB({dest}, {src})"
                    | UXTH (dest, src) -> $"UXTH({dest}, {src})"
                    | UXTW (dest, src) -> $"UXTW({dest}, {src})"
                    ...
                    | TBZ (reg, bit, offset) -> $"TBZ({reg}, {bit}, {offset})"
                    | TBNZ (reg, bit, offset) -> $"TBNZ({reg}, {bit}, {offset})"
                    | TBZ_label (reg, bit, label) -> $"TBZ_label({reg}, {bit}, {label})"
                    | TBNZ_label (reg, bit, label) -> $"TBNZ_label({reg}, {bit}, {label})"
                    | B offset -> $"B({offset})"
                    ...
                    | STP_pre (reg1, reg2, addr, offset) -> $"STP_pre({reg1}, {reg2}, {addr}, {offset})"
                    | LDP (reg1, reg2, addr, offset) -> $"LDP({reg1}, {reg2}, {addr}, {offset})"
                    | LDP_post (reg1, reg2, addr, offset) -> $"LDP_post({reg1}, {reg2}, {addr}, {offset})"
                    ...
                    | FMOV_imm (dest, value) -> $"FMOV_imm({dest}, {value})"
