...
/// can therefore be placed in an ordinary top-level function. Keeping the
/// prepared source avoids reparsing it while execution batches are assembled.
...
/// AggregateRun owns the physical timing; per-test runs divide that timing so
/// timing reports continue to add up to wall-clock work.
type E2EBatchExecution = {
    AggregateRun: E2ERun
    Results: (E2ETest * E2ETestResult) list
}

/// Bound both generated program size and failure scope. JSON checks run behind
/// a closure-calling helper so the production inliner keeps their bodies out
/// of the entry function and avoids monolithic stack frames.
let maxBatchSize = 16
...
let tryPrepareBatchTest (test: E2ETest) : PreparedE2EBatchTest option =
    let eligibleExpectation =
        isJsonBatchFile test.SourceFile
...
let tryParseBatchBoolResults
    ...
    | Some line ->
        ...
        | true, mask when expectedCount > 0 && expectedCount <= maxBatchSize ->
...
let runE2ETestBatchWithPreambleContext
    ...
    | first :: _ ->
        ...
        let results =
            ...
            | _ ->
                // A batch is the sole physical execution. Preserve that
                // contract on failure instead of retrying members individually.
                let message =
                    match aggregateRun with
