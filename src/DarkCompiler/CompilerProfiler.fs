// CompilerProfiler.fs - Lightweight compiler profiling utilities
//
// Records timestamped compiler events with thread IDs for offline analysis.

module CompilerProfiler
...
let recordFunction (passName: string) (funcName: string) (typeArgs: string list) (meta: (string * string) list) : unit =
    if isEnabled() then
        let typeArgsValue =
            if List.isEmpty typeArgs then "" else String.concat "," typeArgs
        let startMs = nowMs()
        let fullMeta =
            [ ("pass", passName)
              ("function", funcName)
              ("type_args", typeArgsValue) ]
            @ meta
        record "function_compiled" startMs startMs fullMeta
