...
let private listRefCountDecTuple2DynamicBothHelperLabel = "__dark_list_refcount_dec_tuple2_dynamic_both_helper"
let private listRefCountDecTuple2List0HelperLabel = "__dark_list_refcount_dec_tuple2_list0_helper"
let private listRefCountDecTuple2List1HelperLabel = "__dark_list_refcount_dec_tuple2_list1_helper"
let private listRefCountDecTuple2ListBothHelperLabel = "__dark_list_refcount_dec_tuple2_list_both_helper"
let private listRefCountDecTuple2Dict0HelperLabel = "__dark_list_refcount_dec_tuple2_dict0_helper"
let private listRefCountDecTuple2Dict1HelperLabel = "__dark_list_refcount_dec_tuple2_dict1_helper"
let private listRefCountDecTuple2DictBothHelperLabel = "__dark_list_refcount_dec_tuple2_dict_both_helper"
let private listRefCountDecTuple2Closure0HelperLabel = "__dark_list_refcount_dec_tuple2_closure0_helper"
...
let private listRefCountDecStringBytesClosureHelperLabel = "__dark_list_refcount_dec_string_bytes_closure_helper"
let private listRefCountDecStringBytesRecordManagedHelperLabel = "__dark_list_refcount_dec_string_bytes_record_managed_helper"
let private listRefCountDecStringBytesRecordManagedDict3HelperLabel = "__dark_list_refcount_dec_string_bytes_record_managed_dict3_helper"
...
let private listRefCountDecSumListHelperLabel = "__dark_list_refcount_dec_sum_list_helper"
...
let private generateListRefCountDecHelperWith
    ...
    let releaseDynamicBufferLeafField (fieldOffset: int) : ARM64Symbolic.Instr list =
        ...
        [
            ...
            ARM64Symbolic.LDR (ARM64Symbolic.X12, ARM64Symbolic.X11, int16 fieldOffset)
    ...
    let leafListHelperLabel (fieldType: AST.Type) : string =
    ...
    let releaseListLeafField (fieldOffset: int) (fieldType: AST.Type) : ARM64Symbolic.Instr list =
        ...
        let callInstrs = [
            ...
            ARM64Symbolic.STP (ARM64Symbolic.X8, ARM64Symbolic.X9, ARM64Symbolic.SP, 64s)
            ARM64Symbolic.STR (ARM64Symbolic.X30, ARM64Symbolic.SP, 80s)
            ARM64Symbolic.STR (ARM64Symbolic.X29, ARM64Symbolic.SP, 88s)
            ARM64Symbolic.MOV_reg (ARM64Symbolic.X29, ARM64Symbolic.SP)
            ARM64Symbolic.MOV_reg (ARM64Symbolic.X0, ARM64Symbolic.X8)
            ARM64Symbolic.BL helperLabel
            ARM64Symbolic.ADD_imm (ARM64Symbolic.SP, ARM64Symbolic.X29, 0us)
            ARM64Symbolic.LDR (ARM64Symbolic.X29, ARM64Symbolic.SP, 88s)
            ARM64Symbolic.LDR (ARM64Symbolic.X30, ARM64Symbolic.SP, 80s)
            ARM64Symbolic.LDP (ARM64Symbolic.X8, ARM64Symbolic.X9, ARM64Symbolic.SP, 64s)
            ARM64Symbolic.LDP (ARM64Symbolic.X6, ARM64Symbolic.X7, ARM64Symbolic.SP, 48s)
            ARM64Symbolic.LDP (ARM64Symbolic.X4, ARM64Symbolic.X5, ARM64Symbolic.SP, 32s)
        ...
        [
            ...
            ARM64Symbolic.CBZ (ARM64Symbolic.X8, fieldDone)
        ]
        @ callInstrs
        @ [ARM64Symbolic.Label fieldDone]
    ...
    let releaseClosureLeafField (fieldOffset: int) : ARM64Symbolic.Instr list =
        let fieldDone = label $"leaf_closure_field_{fieldOffset}_done"
        let callInstrs = [
            ARM64Symbolic.STP_pre (ARM64Symbolic.X0, ARM64Symbolic.X1, ARM64Symbolic.SP, -96s)
            ARM64Symbolic.STP (ARM64Symbolic.X2, ARM64Symbolic.X3, ARM64Symbolic.SP, 16s)
            ARM64Symbolic.STP (ARM64Symbolic.X4, ARM64Symbolic.X5, ARM64Symbolic.SP, 32s)
    ...
    let releaseDynamicBufferFixedLeafChildField (fieldOffset: int) (childOffset: int) : ARM64Symbolic.Instr list =
        ...
        [
            ...
            ARM64Symbolic.CMP_reg (ARM64Symbolic.X12, ARM64Symbolic.X27)
            ARM64Symbolic.B_cond_label (ARM64Symbolic.LT, childDone)
            ARM64Symbolic.CMP_reg (ARM64Symbolic.X12, ARM64Symbolic.X28)
            ARM64Symbolic.B_cond_label (ARM64Symbolic.GT, childDone)
            ARM64Symbolic.LDR (ARM64Symbolic.X14, ARM64Symbolic.X12, 0s)
            ARM64Symbolic.ADD_imm (ARM64Symbolic.X14, ARM64Symbolic.X14, 7us)
            ARM64Symbolic.MOVZ (ARM64Symbolic.X13, 3us, 0)
            ARM64Symbolic.LSR_reg (ARM64Symbolic.X14, ARM64Symbolic.X14, ARM64Symbolic.X13)
            ...
            ARM64Symbolic.MOVK (ARM64Symbolic.X15, 0x7FFFus, 48)
            ARM64Symbolic.CMP_reg (ARM64Symbolic.X14, ARM64Symbolic.X15)
            ARM64Symbolic.B_cond_label (ARM64Symbolic.EQ, childDone)
        ]
        @ refcountUpdate
    ...
    let releaseFixedBlockLeafField (fieldOffset: int) (fieldType: AST.Type) : ARM64Symbolic.Instr list =
        ...
        | Some payloadSize ->
            ...
            [
                ...
                ARM64Symbolic.CBNZ (ARM64Symbolic.X9, fieldDone)
            ]
            @ childReleases
    ...
    let releaseLeafPayload =
        ...
        | Some payloadSize, _, _, _ ->
            ...
            @ (if List.isEmpty releaseManagedLeafFields then
                []
               else
...
let private isStringBytesListFieldShape (fieldTypes: AST.Type list) : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TList AST.TInt64 ] -> true
    | _ -> false

let private isStringBytesDictFieldShape (fieldTypes: AST.Type list) : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TDict (AST.TInt64, AST.TInt64) ] -> true
    | _ -> false

let private isStringBytesClosureFieldShape (fieldTypes: AST.Type list) : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TFunction ([ AST.TInt64 ], AST.TInt64) ] -> true
    | _ -> false

let private isStringBytesRecordManagedFieldShape
    (recordRegistry: LIR.RecordRegistry)
    (fieldTypes: AST.Type list)
    : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TRecord (recordName, _) ] ->
        recordRegistry
        |> Map.tryFind recordName
        |> Option.map (fun fields ->
            match fields |> List.map snd with
            | [ AST.TList AST.TInt64; dynamicBuffer ] ->
...
let private isStringBytesRecordManagedDict3FieldShape
    (recordRegistry: LIR.RecordRegistry)
    (fieldTypes: AST.Type list)
    : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TRecord (recordName, _) ] ->
        ...
        |> Option.map (fun fields ->
            match fields |> List.map snd with
            | [ AST.TDict (AST.TInt64, AST.TInt64); dynamicBuffer ] ->
                isDynamicBufferType dynamicBuffer
            | _ ->
                false)
        |> Option.defaultValue false
...
let private isStringBytesRecordManagedClosure3FieldShape
    (recordRegistry: LIR.RecordRegistry)
    (fieldTypes: AST.Type list)
    : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TRecord (recordName, _) ] ->
        ...
        |> Option.map (fun fields ->
            match fields |> List.map snd with
            | [ AST.TFunction ([ AST.TInt64 ], AST.TInt64); dynamicBuffer ] ->
                isDynamicBufferType dynamicBuffer
            | _ ->
                false)
        |> Option.defaultValue false
...
let private isStringBytesListDictFieldShape (fieldTypes: AST.Type list) : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TList AST.TInt64; AST.TDict (AST.TInt64, AST.TInt64) ] -> true
    | _ -> false

let private isStringBytesListDictRecordList (recordRegistry: LIR.RecordRegistry) (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TRecord (name, _))) ->
        recordRegistry
        |> Map.tryFind name
        |> Option.map (fun fields -> fields |> List.map snd |> isStringBytesListDictFieldShape)
        |> Option.defaultValue false
    | _ ->
        false
...
let private isClosureStringListDictRecordList (recordRegistry: LIR.RecordRegistry) (sourceType: AST.Type option) : bool =
    ...
    | Some (AST.TList (AST.TRecord (name, _))) ->
        recordRegistry
        |> Map.tryFind name
        |> Option.map (fun fields -> fields |> List.map snd |> isClosureStringListDictFieldShape)
        |> Option.defaultValue false
    | _ ->
        false

let private isStringBytesTupleListFieldShape (fieldTypes: AST.Type list) : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TTuple [ AST.TInt64; dynamicBuffer ]; AST.TList AST.TInt64 ] ->
        isDynamicBufferType dynamicBuffer
    | _ -> false

let private isStringBytesTupleDynamic3FieldShape (fieldTypes: AST.Type list) : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TTuple [ AST.TInt64; dynamicBuffer ] ] ->
        isDynamicBufferType dynamicBuffer
    | _ -> false

let private isStringBytesTupleManaged3ListFieldShape (fieldTypes: AST.Type list) : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TTuple [ AST.TList AST.TInt64; dynamicBuffer ] ] ->
        isDynamicBufferType dynamicBuffer
    | _ -> false

let private isStringBytesTupleManaged3DictFieldShape (fieldTypes: AST.Type list) : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TTuple [ AST.TDict (AST.TInt64, AST.TInt64); dynamicBuffer ] ] ->
        isDynamicBufferType dynamicBuffer
    | _ -> false

let private isStringBytesTupleManaged3ClosureFieldShape (fieldTypes: AST.Type list) : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TTuple [ AST.TFunction ([ AST.TInt64 ], AST.TInt64); dynamicBuffer ] ] ->
        isDynamicBufferType dynamicBuffer
    | _ -> false
...
let private isStringBytesTupleManagedDictFieldShape (fieldTypes: AST.Type list) : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TTuple [ AST.TDict (AST.TInt64, AST.TInt64); dynamicBuffer ]; AST.TList AST.TInt64 ] ->
        isDynamicBufferType dynamicBuffer
    | _ -> false

let private isStringBytesTupleManagedClosureFieldShape (fieldTypes: AST.Type list) : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TTuple [ AST.TFunction ([ AST.TInt64 ], AST.TInt64); dynamicBuffer ]; AST.TList AST.TInt64 ] ->
        isDynamicBufferType dynamicBuffer
    | _ -> false

let private isStringBytesRecordManagedListFieldShape
    (recordRegistry: LIR.RecordRegistry)
    (fieldTypes: AST.Type list)
    : bool =
    match fieldTypes with
    | [ AST.TString; AST.TBytes; AST.TRecord (recordName, _); AST.TList AST.TInt64 ] ->
        recordRegistry
        |> Map.tryFind recordName
        |> Option.map (fun fields ->
            match fields |> List.map snd with
...
let private isStringBytesRecordManagedDictFieldShape
    (recordRegistry: LIR.RecordRegistry)
    (fieldTypes: AST.Type list)
    : bool =
    match fieldTypes with
...
let private isStringBytesRecordManagedClosureFieldShape
    (recordRegistry: LIR.RecordRegistry)
    (fieldTypes: AST.Type list)
    : bool =
    match fieldTypes with
...
let private isRecordListDictList (recordRegistry: LIR.RecordRegistry) (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TRecord (name, _))) ->
        recordRegistry
        |> Map.tryFind name
        |> Option.map isSingleListDictFieldShape
        |> Option.defaultValue false
    | _ ->
        false
...
let private isStringBytesListTupleList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesListFieldShape fieldTypes
    | _ -> false

let private isStringBytesDictTupleList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesDictFieldShape fieldTypes
    | _ -> false

let private isStringBytesClosureTupleList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesClosureFieldShape fieldTypes
    | _ -> false

let private isStringBytesRecordManagedTupleList
    (recordRegistry: LIR.RecordRegistry)
    (sourceType: AST.Type option)
    : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesRecordManagedFieldShape recordRegistry fieldTypes
    | _ ->
        false

let private isStringBytesRecordManagedDict3TupleList
    (recordRegistry: LIR.RecordRegistry)
    (sourceType: AST.Type option)
    : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesRecordManagedDict3FieldShape recordRegistry fieldTypes
    | _ ->
        false

let private isStringBytesRecordManagedClosure3TupleList
    (recordRegistry: LIR.RecordRegistry)
    (sourceType: AST.Type option)
    : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesRecordManagedClosure3FieldShape recordRegistry fieldTypes
    | _ ->
        false

let private isStringBytesListDictTupleList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesListDictFieldShape fieldTypes
    | _ -> false

let private isClosureStringListDictTupleList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isClosureStringListDictFieldShape fieldTypes
    | _ -> false

let private isStringBytesTupleListTupleList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesTupleListFieldShape fieldTypes
    | _ -> false

let private isStringBytesTupleDynamic3TupleList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesTupleDynamic3FieldShape fieldTypes
    | _ -> false

let private isStringBytesTupleManaged3ListTupleList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesTupleManaged3ListFieldShape fieldTypes
    | _ -> false

let private isStringBytesTupleManaged3DictTupleList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesTupleManaged3DictFieldShape fieldTypes
    | _ -> false

let private isStringBytesTupleManaged3ClosureTupleList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesTupleManaged3ClosureFieldShape fieldTypes
    | _ -> false
...
let private isStringBytesTupleManagedDictTupleList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesTupleManagedDictFieldShape fieldTypes
    | _ -> false

let private isStringBytesTupleManagedClosureTupleList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesTupleManagedClosureFieldShape fieldTypes
    | _ -> false

let private isStringBytesRecordManagedListTupleList
    (recordRegistry: LIR.RecordRegistry)
    (sourceType: AST.Type option)
    : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesRecordManagedListFieldShape recordRegistry fieldTypes
    | _ ->
        false

let private isStringBytesRecordManagedDictTupleList
    (recordRegistry: LIR.RecordRegistry)
    (sourceType: AST.Type option)
    : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesRecordManagedDictFieldShape recordRegistry fieldTypes
    | _ ->
        false

let private isStringBytesRecordManagedClosureTupleList
    (recordRegistry: LIR.RecordRegistry)
    (sourceType: AST.Type option)
    : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fieldTypes)) ->
        isStringBytesRecordManagedClosureFieldShape recordRegistry fieldTypes
    | _ ->
        false
...
let private tuple2DynamicFieldPattern (fields: AST.Type list) : (bool * bool) option =
    match fields with
    | [ first; second ] ->
        match isDynamicBufferType first, isDynamicBufferType second with
        | false, false -> None
        | pattern -> Some pattern
    | _ ->
        None

let private tuple2DynamicHelperLabel (fields: AST.Type list) : string option =
    match tuple2DynamicFieldPattern fields with
    | Some (true, true) -> Some listRefCountDecTuple2DynamicBothHelperLabel
    | Some (true, false) -> Some listRefCountDecTuple2Dynamic0HelperLabel
    | Some (false, true) -> Some listRefCountDecTuple2Dynamic1HelperLabel
    | Some (false, false)
    | None -> None

let private isListType (typ: AST.Type) : bool =
    match typ with
    | AST.TList _ -> true
    | _ -> false

let private tuple2ListFieldPattern (fields: AST.Type list) : (bool * bool) option =
    match fields with
    | [ first; second ] ->
        match isListType first, isListType second with
        | false, false -> None
        | pattern -> Some pattern
    | _ ->
        None

let private tuple2ListHelperLabel (fields: AST.Type list) : string option =
    match tuple2ListFieldPattern fields with
    | Some (true, true) -> Some listRefCountDecTuple2ListBothHelperLabel
    | Some (true, false) -> Some listRefCountDecTuple2List0HelperLabel
    | Some (false, true) -> Some listRefCountDecTuple2List1HelperLabel
    | Some (false, false)
    | None -> None

let private isDictType (typ: AST.Type) : bool =
    match typ with
    | AST.TDict _ -> true
    | _ -> false

let private tuple2DictFieldPattern (fields: AST.Type list) : (bool * bool) option =
    match fields with
    | [ first; second ] ->
        match isDictType first, isDictType second with
        | false, false -> None
        | pattern -> Some pattern
    | _ ->
        None

let private tuple2DictHelperLabel (fields: AST.Type list) : string option =
    match tuple2DictFieldPattern fields with
    | Some (true, true) -> Some listRefCountDecTuple2DictBothHelperLabel
    | Some (true, false) -> Some listRefCountDecTuple2Dict0HelperLabel
    | Some (false, true) -> Some listRefCountDecTuple2Dict1HelperLabel
    | Some (false, false)
    | None -> None

let private isFunctionType (typ: AST.Type) : bool =
    match typ with
    | AST.TFunction _ -> true
    | _ -> false

let private tuple2ClosureFieldPattern (fields: AST.Type list) : (bool * bool) option =
    match fields with
    | [ first; second ] ->
        match isFunctionType first, isFunctionType second with
        | false, false -> None
        | pattern -> Some pattern
    | _ ->
        None

let private tuple2ClosureHelperLabel (fields: AST.Type list) : string option =
    match tuple2ClosureFieldPattern fields with
    | Some (true, true) -> Some listRefCountDecTuple2ClosureBothHelperLabel
    | Some (true, false) -> Some listRefCountDecTuple2Closure0HelperLabel
    | Some (false, true) -> Some listRefCountDecTuple2Closure1HelperLabel
    | Some (false, false)
    | None -> None

let private isTuple2DynamicList (sourceType: AST.Type option) : bool =
    match sourceType with
...
let private isTuple2ListList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fields)) ->
...
let private isTuple2DictList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fields)) ->
...
let private isTuple2ClosureList (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TTuple fields)) ->
...
let private isClosurePayloadSumList (variantRegistry: LIR.VariantRegistry) (sourceType: AST.Type option) : bool =
    match sourceType with
    | Some (AST.TList (AST.TSum (_, [AST.TFunction _]))) -> true
    | _ ->
        sumPayloadsAllMatch
            variantRegistry
            sourceType
...
let private isListStringFixedBlockPayload (recordRegistry: LIR.RecordRegistry) (payloadType: AST.Type) : bool =
    match payloadType with
    | AST.TTuple [ AST.TList AST.TInt64; AST.TString ] ->
        true
    | AST.TRecord (name, _) ->
        ...
        |> Option.map (fun fields ->
            match fields |> List.map snd with
            | [ AST.TList AST.TInt64; AST.TString ] ->
                true
            | _ ->
                false)
        |> Option.defaultValue false
...
let private isDictStringFixedBlockPayload (recordRegistry: LIR.RecordRegistry) (payloadType: AST.Type) : bool =
    match payloadType with
    | AST.TTuple [ AST.TDict (AST.TInt64, AST.TInt64); AST.TString ] ->
        true
    | AST.TRecord (name, _) ->
        ...
        |> Option.map (fun fields ->
            match fields |> List.map snd with
            | [ AST.TDict (AST.TInt64, AST.TInt64); AST.TString ] ->
                true
            | _ ->
                false)
        |> Option.defaultValue false
...
let private isClosureStringFixedBlockPayload (recordRegistry: LIR.RecordRegistry) (payloadType: AST.Type) : bool =
    match payloadType with
    | AST.TTuple [ AST.TFunction _; AST.TString ] ->
        true
    | AST.TRecord (name, _) ->
        recordRegistry
        |> Map.tryFind name
...
let private isTuple4NestedTuplePayload (recordRegistry: LIR.RecordRegistry) (payloadType: AST.Type) : bool =
    ...
    | Some [
        ...
        nestedPayload
        AST.TList AST.TInt64
      ] when isListStringFixedBlockPayload recordRegistry nestedPayload ->
        true
    | _ ->
        false
...
let private isTuple4NestedDictPayload (recordRegistry: LIR.RecordRegistry) (payloadType: AST.Type) : bool =
    ...
    | Some [
        ...
        nestedPayload
        AST.TList AST.TInt64
      ] when isDictStringFixedBlockPayload recordRegistry nestedPayload ->
        true
    | _ ->
        false
...
let private isTuple4NestedClosurePayload (recordRegistry: LIR.RecordRegistry) (payloadType: AST.Type) : bool =
    ...
    | Some [
        ...
        nestedPayload
        AST.TList AST.TInt64
      ] when isClosureStringFixedBlockPayload recordRegistry nestedPayload ->
        true
    | _ ->
        false
...
let private isTuple4NestedTuplePayloadSumList
    ...
    : bool =
    match sourceType with
    | Some (AST.TList (AST.TSum (_, [ payloadType ]))) when isTuple4NestedTuplePayload recordRegistry payloadType ->
        true
    | _ ->
        sumPayloadsAllMatch variantRegistry sourceType (isTuple4NestedTuplePayload recordRegistry)

let private isTuple4NestedDictPayloadSumList
    (recordRegistry: LIR.RecordRegistry)
    (variantRegistry: LIR.VariantRegistry)
    (sourceType: AST.Type option)
    : bool =
    match sourceType with
    | Some (AST.TList (AST.TSum (_, [ payloadType ]))) when isTuple4NestedDictPayload recordRegistry payloadType ->
        true
    | _ ->
        sumPayloadsAllMatch variantRegistry sourceType (isTuple4NestedDictPayload recordRegistry)

let private isTuple4NestedClosurePayloadSumList
    (recordRegistry: LIR.RecordRegistry)
    (variantRegistry: LIR.VariantRegistry)
    (sourceType: AST.Type option)
    : bool =
    match sourceType with
    | Some (AST.TList (AST.TSum (_, [ payloadType ]))) when isTuple4NestedClosurePayload recordRegistry payloadType ->
        true
    | _ ->
        sumPayloadsAllMatch variantRegistry sourceType (isTuple4NestedClosurePayload recordRegistry)
...
let private normalizedFieldReleases (fieldReleases: ANF.RcFieldRelease list) : ANF.RcFieldRelease list =
    fieldReleases
...
let private managedLeafFieldReleases
    (payloadSize: int)
    (fieldTypes: AST.Type list)
    : ANF.RcFieldRelease list option =
    match fieldTypes with
    | [] ->
        Some []
    | _ ->
...
let private fixedBlockListHelperFromSpecs
    (payloadSize: int)
    (fieldReleases: ANF.RcFieldRelease list)
    : string option =
    let expectedFieldReleases = normalizedFieldReleases fieldReleases

    listRefCountDecHelperSpecs
    |> List.tryPick (fun spec ->
        match spec.LeafGenericPayloadSize, spec.ReleaseLeafListPayload, spec.ReleaseLeafDictPayload, spec.ReleaseLeafClosurePayload with
        | Some specPayloadSize, false, false, false when specPayloadSize = payloadSize ->
            spec.ManagedLeafFieldTypes
            |> managedLeafFieldReleases payloadSize
            |> Option.bind (fun specFieldReleases ->
                if normalizedFieldReleases specFieldReleases = expectedFieldReleases then
                    Some spec.Label
                else
                    None)
        | _ ->
...
let private fixedBlockListHelperForPayloadRelease
    ...
    (fieldReleases: ANF.RcFieldRelease list)
    : string =
    match fixedBlockListHelperFromSpecs payloadSize fieldReleases with
    | Some helperLabel ->
        helperLabel
    | None ->
