...
type LambdaEnv = Map<AST.BindingId, CheckedAST.Expr>

/// Check if a variable occurs in an expression (for dead code elimination)
let rec varOccursInExpr (name: AST.BindingId) (expr: CheckedAST.Expr) : bool =
    match expr with
    ...
    | CheckedAST.RecursiveLet (recursion, value, body) ->
        if CheckedAST.recursiveBindingId recursion = name then false
...
let rec inlineLambdas (expr: CheckedAST.Expr) (lambdaEnv: LambdaEnv) : CheckedAST.Expr =
    ...
    | CheckedAST.Let (pattern, value, body) ->
        ...
        CheckedAST.Let (pattern, value', body')
    | CheckedAST.RecursiveLet (recursion, value, body) ->
        let childEnv = Map.remove (CheckedAST.recursiveBindingId recursion) lambdaEnv
    ...
    | CheckedAST.Constructor (reference, fields) ->
        CheckedAST.Constructor (reference, List.map (fun e -> inlineLambdas e lambdaEnv) fields)
    | CheckedAST.Match (scrutinee, cases) ->
        let cases' =
            cases
            |> List.map (fun mc ->
    ...
    | CheckedAST.Apply (func, args) ->
        ...
        | CheckedAST.Local id ->
            ...
            match Map.tryFind id lambdaEnv with
            | Some _ ->
                CheckedAST.Apply (CheckedAST.Local id, args')
            | None ->
