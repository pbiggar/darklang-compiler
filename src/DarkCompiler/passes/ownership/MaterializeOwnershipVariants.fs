...
type CallSiteIdentity = { Caller: string; Result: HIR.ValueId }
...
let private boundaryCall (definition: Function<'leaf, 'id>) : HIR.FunctionCall = {
    Target = definition.Definition.Name
...
let hirContracts plan (source: VerifyOwnedHIR.HIRContracts<'leaf>) =
    let registry =
        members plan
        |> List.map (fun memberDefinition -> memberDefinition.Function.Definition.Name, memberDefinition)
        |> Map.ofList
    { source with
...
let ownershipSemantics plan (source: Semantics<'leaf, 'id>) =
    let registry =
        ...
        |> List.map (fun memberDefinition ->
            memberDefinition.Function.Definition.Name, verifiedCallSignature memberDefinition.Function.Ownership)
        |> Map.ofList
    { source with
...
let private validateRequests definitions discovered semantics requests =
    ...
    let groupNames =
        ...
        |> List.collect (fun group ->
            ...
            names |> Set.toList |> List.map (fun name -> name, names))
        |> Map.ofList
    let callSites =
        definitions
        |> List.collect (fun definition ->
            calls definition.Definition.Body
            |> List.map (fun call -> { Caller = definition.Definition.Name; Result = call.Result.Id }, call))
        |> Map.ofList
    let validateCandidate request selected =
        ...
        |> Result.bind (fun () ->
            ...
            if target.Name <> request.Call.Target then Error (BoundaryMismatch request.Call.Target)
            elif Map.tryFind target.Name groupNames <> Some names then Error (GroupMembershipMismatch target.Name)
            elif Set.contains request.Caller names then Error (MixedRecursiveCandidate (site request))
            else Ok ())
    requests
                ...
                | Some _ ->
                    let valid =
                        ...
                        | EstablishedBoundary expected ->
                            let actual =
                                match Map.tryFind request.Call.Target definitionsByName with
                                | Some definition ->
                            ...
                            | _ -> Error (BoundaryMismatch request.Call.Target)
...
let private cloneGroups
    ...
    |> List.fold (fun result (identity, candidate) ->
        result |> Result.bind (fun (occupied, groups) ->
            ...
            let symbols = boundaries |> List.map (fun boundary -> boundary.Name, boundary.Name + suffix) |> Map.ofList
                ...
                result |> Result.bind (fun (occupied, members) ->
                    ...
                    | Some original ->
                        ...
                        if Set.contains name occupied
                           || Option.isSome (hir.CallSignature name)
                           || Option.isSome (semantics.CallOwnership (boundaryCall clone)) then
                            Error (SymbolCollision name)
                        else Ok (Set.add name occupied, { Original = boundary.Name; Function = clone } :: members))) (Ok (occupied, []))
    ...
    |> Result.bind (fun groups ->
        let targets =
            ...
            |> List.collect (fun group ->
                ...
                |> List.map (fun memberDefinition ->
                    (group.Identity, memberDefinition.Original), memberDefinition.Function.Definition.Name))
            |> Map.ofList
        let rewrites =
            requests
            |> List.choose (fun request ->
        ...
        let originals =
            definitions |> List.map (fun definition ->
                let rewrite (call: HIR.FunctionCall) =
                    match Map.tryFind { Caller = definition.Definition.Name; Result = call.Result.Id } bySite with
