...
let private allocate layout length vg =
    ...
    match layout with
    | RuntimeArray _ -> emit (ANF.Call ("Darklang.Stdlib.List.__arrayAllocate", [length])) vg
    ...
    let release buffers values vg =
        values |> List.mapFold (fun state value ->
            ...
            let operation =
                ...
                | RuntimeArray _ -> ANF.Call ("Darklang.Stdlib.List.__arrayRelease", [buffer.Pointer])
        ...
        | BorrowAndCopy ->
            ...
            let copied, afterCopy =
                ...
                | MappedArray _ | RuntimeArray _ ->
                    let _, bindings, next = emit (ANF.Call ("Darklang.Stdlib.List.__arrayCopy", [buffer.Pointer; copy; word 0; buffer.Length])) afterAllocation
                    [bindings], next
            ...
            | Leaf (Construct (output, Repeat (count, value))) ->
                lowerValue values vg count |> Result.bind (fun (countExpr, countAtom, afterCount) ->
                    lowerValue values afterCount value |> Result.bind (fun (valueExpr, valueAtom, afterValue) ->
                        let pointer, allocation, afterAllocation = emit (ANF.Call ("Darklang.Stdlib.List.__arrayRepeat", [countAtom; valueAtom])) afterValue
                ...
                callback |> Result.bind (fun (evaluation, fn, next) ->
                    ...
                    let mutations, afterMutation =
                        ...
                        | Map _, (MappedArray _ | RuntimeArray _) ->
                            let _, bindings, next = emit (ANF.Call ("Darklang.Stdlib.List.__arrayMap", [target; word 0; buffer.Length; fn])) afterDestination
                            [bindings], next
                        | Reverse, (MappedArray _ | RuntimeArray _) ->
                            ...
                            let _, bindings, next = emit (ANF.Call ("Darklang.Stdlib.List.__arrayReverse", [target; word 0; last])) afterLast
                ...
                lowerValue values vg initial |> Result.bind (fun (initialExpr, accumulator, next) ->
                    lowerValue values next fn |> Result.bind (fun (callbackExpr, callback, afterCallback) ->
                        ...
                        let bindings, (value, afterFold) =
                            match buffer.Layout with
                            | MappedArray _ | RuntimeArray _ ->
                                let result, calls, next = emit (ANF.Call ("Darklang.Stdlib.List.__arrayFold", [buffer.Pointer; word 0; buffer.Length; accumulator; callback])) afterCallback
                                [calls], (result, next)
                            | RecycledArray length ->
