...
| 6 | Separate source types from compiler/internal types | **Partly done.** `AST.ParsedType` and `AST.SemanticType` are distinct. Checked function and lambda signatures use a private `CheckedSignatureType` that excludes live call-local inference identities. Other checked expression types and downstream IRs still share the broad semantic union; see below. |
...
Checked function and lambda signatures now use the certified type. Their
producers, generated-function paths, alias resolution, specialization, and
HIR/ANF consumers preserve nominal type arguments, `TNever`, and privileged
`TInternalRawPtr` where those remain meaningful. The next type-design step is
to choose an invariant for other checked expression type fields or for backend
representation/ABI classes; callable signatures alone do not certify either
of those boundaries.
