...
| Sorting and uniqueness | `sort`, `sortBy`, `unique`, and `uniqueBy` use canonical typed ordering. `dedup` retains the first occurrence of each value without reordering the survivors. `sortByComparator` is stable, accepts an `Int` comparator, and returns the exact error unless each result is `-1`, `0`, or `1`. |
