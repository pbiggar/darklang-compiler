...
9. After all tests pass, run all Dark benchmarks (`./benchmarks/run_benchmarks.sh all`). Benchmarks should complete with no failures; treat any benchmark failure, including quicksort, as validation evidence to investigate and report. Report the "Performance ratio: X.XX" from the RESULTS.md table header and show the RESULTS.md diff if it changed.
