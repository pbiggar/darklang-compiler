...
9. After all tests pass, run the routine Dark benchmark profile (`./benchmarks/run_benchmarks.sh routine`). Benchmarks should complete with no failures. Report the "Performance ratio: X.XX" from the RESULTS.md table header and show the RESULTS.md diff if it changed. Run full-size quicksort explicitly only when that workload is relevant.
